...
class PhysicsTransientConvectionDiffusion : public PhysicsTransientBase
{
    ...
    */
    ...
    std::vector<Scalar3D*> velocity_x_ptr_vec;
    std::vector<Scalar3D*> velocity_y_ptr_vec;
    std::vector<Scalar3D*> velocity_z_ptr_vec;
    ...
    void set_domain(Domain3D &domain_in, Integral3D &integral_in, Scalar3D &derivativecoefficient_in, Scalar3D &diffusioncoefficient_in, Scalar3D &velocity_x_in, Scalar3D &velocity_y_in, Scalar3D &velocity_z_in, Scalar3D &generationcoefficient_in);
    ...
    (
        ...
        Scalar3D *derivativecoefficient_ptr, Scalar3D *diffusioncoefficient_ptr, Scalar3D *velocity_x_ptr, Scalar3D *velocity_y_ptr, Scalar3D *velocity_z_ptr, Scalar3D *generationcoefficient_ptr
    );
...
void PhysicsTransientConvectionDiffusion::set_domain(Domain3D &domain_in, Integral3D &integral_in, Scalar3D &derivativecoefficient_in, Scalar3D &diffusioncoefficient_in, Scalar3D &velocity_x_in, Scalar3D &velocity_y_in, Scalar3D &velocity_z_in, Scalar3D &generationcoefficient_in)
{
    /*
    ...
    velocity_x_in : Scalar3D
        x-component of v in a * du/dt = -div(-b * grad(u)) - v * grad(u) + c.
    velocity_y_in : Scalar3D
        y-component of v in a * du/dt = -div(-b * grad(u)) - v * grad(u) + c.
    velocity_z_in : Scalar3D
        z-component of v in a * du/dt = -div(-b * grad(u)) - v * grad(u) + c.
    ...
    */
    ...
    scalar3d_ptr_vec.push_back(&velocity_x_in);
    scalar3d_ptr_vec.push_back(&velocity_y_in);
    scalar3d_ptr_vec.push_back(&velocity_z_in);
    ...
    integral_in.evaluate_integral_Ni_derivative_Nj_z();

}

void PhysicsTransientConvectionDiffusion::set_boundary_dirichlet(Domain2D &domain_in, Scalar2D &value_constant_in)
{
    /*
...
void PhysicsTransientConvectionDiffusion::set_boundary_neumann(Domain2D &domain_in, Integral2D &integral_in, Scalar2D &value_flux_in)
{
    /*
...
)
{
    ...
    */
    ...
    for (int indx_d = 0; indx_d < domain_ptr_vec.size(); indx_d++)
    {
        ...
        Scalar3D *velocity_x_ptr = velocity_x_ptr_vec[indx_d];
        Scalar3D *velocity_y_ptr = velocity_y_ptr_vec[indx_d];
        Scalar3D *velocity_z_ptr = velocity_z_ptr_vec[indx_d];
        ...
        matrix_fill_domain(
            ...
            derivativecoefficient_ptr, diffusioncoefficient_ptr, velocity_x_ptr, velocity_y_ptr, velocity_z_ptr, generationcoefficient_ptr
        );
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element; edid++)
    {
        ...
        VectorDouble velz_vec = velocity_z_ptr->get_neighbor_value(edid);
        ...
        for (int indx_j = 0; indx_j < domain_ptr->num_neighbor; indx_j++){
            ...
            delta_a_triplet_vec.push_back(EigenTriplet(
                ...
                velz_vec[indx_i] * integral_ptr->integral_Ni_derivative_Nj_z_vec[edid][indx_i][indx_j]
            ));
