#ifndef MODULE_HEAT_TRANSIENT_HEX8
#define MODULE_HEAT_TRANSIENT_HEX8
#include <vector>
#include "Eigen/Eigen"
...
class HeatTransientHex8Class
{
    ...
    void matrix_fill(
        ...
        ScalarHex8Class &density_sh8c, ScalarHex8Class &heat_capacity_sh8c, ScalarHex8Class &thermal_conductivity_sh8c, ScalarHex8Class &heat_generation_sh8c,
        double dt, int start_id
    );

    // constructor
    HeatTransientHex8Class(GridHex8Struct &gh8s_in, BoundaryHex8Struct &bh8s_in, IntegralHex8Class &ih8c_in)
    {
        ...
        ih8c.evaluate_integral_Ni_hex8_Nj_hex8();
...
void HeatTransientHex8Class::matrix_fill
(
    Eigen::SparseMatrix<double> &a_mat, Eigen::VectorXd &b_vec, Eigen::VectorXd &x_last_iteration_vec, Eigen::VectorXd &x_last_timestep_vec,
    ScalarHex8Class &density_sh8c, ScalarHex8Class &heat_capacity_sh8c, ScalarHex8Class &thermal_conductivity_sh8c, ScalarHex8Class &heat_generation_sh8c,
    double dt, int start_id
)
{
    ...
    Eigen::SparseMatrix<double> help_mat(gh8s.num_point, gh8s.num_point);  // helper matrix used to make b_vec
    ...
    for (int m = 0; m < gh8s.num_element; m++)
    {
        ...
        int density00 = density_sh8c.scalar_vec[n00];
        int density01 = density_sh8c.scalar_vec[n01];
        int density02 = density_sh8c.scalar_vec[n02];
        int density03 = density_sh8c.scalar_vec[n03];
        int density04 = density_sh8c.scalar_vec[n04];
        int density05 = density_sh8c.scalar_vec[n05];
        int density06 = density_sh8c.scalar_vec[n06];
        int density07 = density_sh8c.scalar_vec[n07];
        int density_arr[8] = {density00, density01, density02, density03, density04, density05, density06, density07};

        // get heat capacity of points around element
        int heatcap00 = heat_capacity_sh8c.scalar_vec[n00];
        int heatcap01 = heat_capacity_sh8c.scalar_vec[n01];
        int heatcap02 = heat_capacity_sh8c.scalar_vec[n02];
        int heatcap03 = heat_capacity_sh8c.scalar_vec[n03];
        int heatcap04 = heat_capacity_sh8c.scalar_vec[n04];
        int heatcap05 = heat_capacity_sh8c.scalar_vec[n05];
        int heatcap06 = heat_capacity_sh8c.scalar_vec[n06];
        int heatcap07 = heat_capacity_sh8c.scalar_vec[n07];
        int heatcap_arr[8] = {heatcap00, heatcap01, heatcap02, heatcap03, heatcap04, heatcap05, heatcap06, heatcap07};

        // get thermal conductivity of points around element
        int thermcond00 = thermal_conductivity_sh8c.scalar_vec[n00];
        int thermcond01 = thermal_conductivity_sh8c.scalar_vec[n01];
        int thermcond02 = thermal_conductivity_sh8c.scalar_vec[n02];
        int thermcond03 = thermal_conductivity_sh8c.scalar_vec[n03];
        int thermcond04 = thermal_conductivity_sh8c.scalar_vec[n04];
        int thermcond05 = thermal_conductivity_sh8c.scalar_vec[n05];
        int thermcond06 = thermal_conductivity_sh8c.scalar_vec[n06];
        int thermcond07 = thermal_conductivity_sh8c.scalar_vec[n07];
        int thermcond_arr[8] = {thermcond00, thermcond01, thermcond02, thermcond03, thermcond04, thermcond05, thermcond06, thermcond07};

        // get heat generation of points around element
        int heatgen00 = heat_generation_sh8c.scalar_vec[n00];
        int heatgen01 = heat_generation_sh8c.scalar_vec[n01];
        int heatgen02 = heat_generation_sh8c.scalar_vec[n02];
        int heatgen03 = heat_generation_sh8c.scalar_vec[n03];
        int heatgen04 = heat_generation_sh8c.scalar_vec[n04];
        int heatgen05 = heat_generation_sh8c.scalar_vec[n05];
        int heatgen06 = heat_generation_sh8c.scalar_vec[n06];
        int heatgen07 = heat_generation_sh8c.scalar_vec[n07];
        int heatgen_arr[8] = {heatgen00, heatgen01, heatgen02, heatgen03, heatgen04, heatgen05, heatgen06, heatgen07};
        ...
        for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            a_mat.coeffRef(start_id + n_arr[i], start_id + n_arr[j]) += inverse_dt*density_arr[i]*heatcap_arr[i]*ih8c.integral_Ni_hex8_Nj_hex8_vec[m][i][j] + 0.5*thermcond_arr[i]*ih8c.integral_div_Ni_hex8_dot_div_Nj_hex8_vec[m][i][j];
        }}

        // calculate help_mat coefficients
        for (int i = 0; i < 8; i++){
        for (int j = 0; j < 8; j++){
            help_mat.coeffRef(n_arr[i], n_arr[j]) += inverse_dt*density_arr[i]*heatcap_arr[i]*ih8c.integral_Ni_hex8_Nj_hex8_vec[m][i][j] - 0.5*thermcond_arr[i]*ih8c.integral_div_Ni_hex8_dot_div_Nj_hex8_vec[m][i][j];
        }}

        // calculate b_vec coefficients
        for (int i = 0; i < 8; i++)
        {
            b_vec.coeffRef(start_id + n_arr[i]) += heatgen_arr[i]*ih8c.integral_Ni_hex8_vec[m][i];
        }

    }

    // add additional terms for b_vec
    b_vec += help_mat*x_last_timestep_vec(Eigen::seqN(start_id, gh8s.num_point));
