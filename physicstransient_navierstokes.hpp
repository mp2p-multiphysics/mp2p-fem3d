...
class PhysicsTransientNavierStokes : public PhysicsTransientBase
{
    ...
    NSE x-component: rho * (du/dt + u * du/dx + v * du/dy + w * du/dz) = -dp/dx + mu * (d^2(u)/dx^2 + d^2(u)/dy^2 + d^2(u)/dz^2) + rho * g_x
    NSE y-component: rho * (dv/dt + u * dv/dx + v * dv/dy + w * dv/dz) = -dp/dy + mu * (d^2(v)/dx^2 + d^2(v)/dy^2 + d^2(v)/dz^2) + rho * g_y
    NSE z-component: rho * (dw/dt + u * dw/dx + v * dw/dy + w * dw/dz) = -dp/dz + mu * (d^2(w)/dx^2 + d^2(w)/dy^2 + d^2(w)/dz^2) + rho * g_z
...
void PhysicsTransientNavierStokes::set_variablegroup(VariableGroup &velocity_x_in, VariableGroup &velocity_y_in, VariableGroup &velocity_z_in, VariableGroup &pressure_in)
{
    /*
...
void PhysicsTransientNavierStokes::set_domain(Domain3D &domain_line_in, Domain3D &domain_quad_in, IntegralTaylorHood3D &integral_in, Scalar3D &density_in, Scalar3D &viscosity_in, Scalar3D &force_x_in, Scalar3D &force_y_in, Scalar3D &force_z_in)
{
    /*
...
void PhysicsTransientNavierStokes::set_boundary_velocity(Domain2D &domain_quad_in, Scalar2D &velocity_x_in, Scalar2D &velocity_y_in, Scalar2D &velocity_z_in)
{
    /*
...
void PhysicsTransientNavierStokes::set_boundary_pressure(Domain2D &domain_line_in, Scalar2D &pressure_in)
{
    /*
...
)
{
   ...
   for (int indx_d = 0; indx_d < domain_line_ptr_vec.size(); indx_d++)
   {
        ...
        matrix_fill_domain(delta_a_triplet_vec, delta_c_triplet_vec, d_vec, x_vec, x_last_timestep_vec, dt, domain_line_ptr, domain_quad_ptr, integral_ptr, density_ptr, viscosity_ptr, force_x_ptr, force_y_ptr, force_z_ptr);
    ...
    for (int indx_d = 0; indx_d < velocity_domain_ptr_vec.size(); indx_d++)
    {
        ...
        matrix_fill_velocity(a_mat, c_mat, d_vec, x_vec, x_last_timestep_vec, dt, domain_ptr, velocity_x_ptr, velocity_y_ptr, velocity_z_ptr);
    }
    for (int indx_d = 0; indx_d < pressure_domain_ptr_vec.size(); indx_d++)
    {
        Domain2D *domain_ptr = pressure_domain_ptr_vec[indx_d];
...
)
{
    ...
    for (int edid = 0; edid < domain_quad_ptr->num_element; edid++)
    {
        ...
        {
            int mat_row = start_row + offset_nsez + velz_pfid_vec[indx_i];
            d_vec.coeffRef(mat_row) += den_vec[indx_i] * fcez_vec[indx_i] * integral_ptr->integral_Mi_quad_vec[edid][indx_i];
        }
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element; edid++)
    {
        ...
        for (int indx_i = 0; indx_i < domain_ptr->num_neighbor; indx_i++)
        {
            ...
            c_mat.row(velz_mat_row) *= 0.;
            d_vec.coeffRef(velz_mat_row) = 0.;
        }
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element; edid++)
    {
        ...
        for (int indx_i = 0; indx_i < domain_ptr->num_neighbor; indx_i++)
        {
            ...
            a_mat.coeffRef(mat_row, mat_col) += 1.;
            d_vec.coeffRef(mat_row) += velz_vec[indx_i];
        }
